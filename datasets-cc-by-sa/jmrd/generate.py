...
def process_file(data, split:str):
    ...
    for i in range(forward_n):
        file_name = os.path.join(file_dir, "data", f"{split}_{i:0>6}.json")
        json.dump(
...
process_file(train_data, 'train')
process_file(valid_data, 'valid')
process_file(test_data, 'test')